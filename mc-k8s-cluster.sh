...
if [[ "${PLATFORM^^}" == "AWS" ]] ; then 
    ...
    if command -v $HOME/bin/az >/dev/null 2>&1; then
        echo
...
export AZURE_NODE_TYPE=Standard_D3_v2
...
export AZ_RESOURCEGROUP=asm-aks-cluster-rg
echo
...
case ${PLATFORM^^} in
    ...
    AZURE)
        if [ $MODE -eq 1 ]; then
            ...
            export STEP="${STEP},2"
            echo
            echo "$ curl -L https://aka.ms/InstallAzureCli | bash # to install CLI" | pv -qL 100
            ...
            export STEP="${STEP},2x"
            echo
            echo "$ sudo rm -rf $PROJDIR # to delete folder" | pv -qL 100
            sudo rm -rf $PROJDIR
        fi
    ;;
    GCP)
...
case ${PLATFORM^^} in
    ...
    AZURE)
        if [ $MODE -eq 1 ]; then
            export STEP="${STEP},3i"
            echo
            echo "$ $HOME/bin/az login --use-device-code # to log on to Azure account" | pv -qL 100
        elif [ $MODE -eq 2 ]; then
            export STEP="${STEP},3"
            echo
            echo "$ $HOME/bin/az login --use-device-code # to log on to Azure account" | pv -qL 100
            $HOME/bin/az login --use-device-code
        elif [ $MODE -eq 3 ]; then
            export STEP="${STEP},3x"
            echo
            echo "*** Nothing to delete ***" | pv -qL 100
...
case ${PLATFORM^^} in
    ...
    AZURE)
        if [ $MODE -eq 1 ]; then
            export STEP="${STEP},4i"
            echo
            echo "$ $HOME/bin/az group create -l \${AZURE_LOCATION} -n \${AZ_RESOURCEGROUP} # create resource group" | pv -qL 100
            echo
            echo "$ $HOME/bin/az account set -s \$($HOME/bin/az account list --query '[?isDefault].id' -o tsv) # to set account" | pv -qL 100
            echo
            echo "$ $HOME/bin/az configure --defaults location=\${AZURE_LOCATION} # to configure location" | pv -qL 100
            echo
            echo "$ $HOME/bin/az configure --defaults group=\${AZ_RESOURCEGROUP} # to configure group" | pv -qL 100
            echo
            echo "$ gcloud --project \$GCP_PROJECT services enable gkemulticloud.googleapis.com connectgateway.googleapis.com cloudresourcemanager.googleapis.com container.googleapis.com gkeconnect.googleapis.com gkehub.googleapis.com serviceusage.googleapis.com anthos.googleapis.com logging.googleapis.com monitoring.googleapis.com stackdriver.googleapis.com storage-api.googleapis.com storage-component.googleapis.com securetoken.googleapis.com sts.googleapis.com anthos.googleapis.com opsconfigmonitoring.googleapis.com # to enable APIs" | pv -qL 100
        elif [ $MODE -eq 2 ]; then
            export STEP="${STEP},4"
            echo
            echo "$ $HOME/bin/az group create -l ${AZURE_LOCATION} -n ${AZ_RESOURCEGROUP} # create resource group" | pv -qL 100
            $HOME/bin/az group create -l ${AZURE_LOCATION} -n ${AZ_RESOURCEGROUP}
            echo
            echo "$ $HOME/bin/az account set -s \$($HOME/bin/az account list --query '[?isDefault].id' -o tsv) # to set account" | pv -qL 100
            $HOME/bin/az account set -s $($HOME/bin/az account list --query '[?isDefault].id' -o tsv)
            echo
            echo "$ $HOME/bin/az configure --defaults location=${AZURE_LOCATION} # to configure location" | pv -qL 100
            $HOME/bin/az configure --defaults location=${AZURE_LOCATION}
            echo
            echo "$ $HOME/bin/az configure --defaults group=${AZ_RESOURCEGROUP} # to configure group" | pv -qL 100
            $HOME/bin/az configure --defaults group=${AZ_RESOURCEGROUP}
            echo
            echo "$ gcloud --project $GCP_PROJECT services enable gkemulticloud.googleapis.com connectgateway.googleapis.com cloudresourcemanager.googleapis.com container.googleapis.com gkeconnect.googleapis.com gkehub.googleapis.com serviceusage.googleapis.com anthos.googleapis.com logging.googleapis.com monitoring.googleapis.com stackdriver.googleapis.com storage-api.googleapis.com storage-component.googleapis.com securetoken.googleapis.com sts.googleapis.com anthos.googleapis.com opsconfigmonitoring.googleapis.com # to enable APIs" | pv -qL 100
            gcloud --project $GCP_PROJECT services enable gkemulticloud.googleapis.com connectgateway.googleapis.com cloudresourcemanager.googleapis.com container.googleapis.com gkeconnect.googleapis.com gkehub.googleapis.com serviceusage.googleapis.com anthos.googleapis.com logging.googleapis.com monitoring.googleapis.com stackdriver.googleapis.com storage-api.googleapis.com storage-component.googleapis.com securetoken.googleapis.com sts.googleapis.com anthos.googleapis.com opsconfigmonitoring.googleapis.com
       elif [ $MODE -eq 3 ]; then
            export STEP="${STEP},4x"
            echo
            echo "$ $HOME/bin/az group delete --name ${AZ_RESOURCEGROUP} --yes # delete resource group" | pv -qL 100
            $HOME/bin/az group delete --name ${AZ_RESOURCEGROUP} --yes >/dev/null 2>&1
        else
            export STEP="${STEP},4i"
            echo
    ...
    GCP)
        if [ $MODE -eq 1 ]; then
            export STEP="${STEP},4i"
            echo
            echo "$ gcloud --project \$GCP_PROJECT services enable gkemulticloud.googleapis.com connectgateway.googleapis.com cloudresourcemanager.googleapis.com container.googleapis.com gkeconnect.googleapis.com gkehub.googleapis.com serviceusage.googleapis.com anthos.googleapis.com logging.googleapis.com monitoring.googleapis.com stackdriver.googleapis.com storage-api.googleapis.com storage-component.googleapis.com securetoken.googleapis.com sts.googleapis.com anthos.googleapis.com # to enable APIs" | pv -qL 100
         elif [ $MODE -eq 2 ]; then
            export STEP="${STEP},4"
            gcloud config set project $GCP_PROJECT > /dev/null 2>&1
            echo
            echo "$ gcloud --project $GCP_PROJECT services enable gkemulticloud.googleapis.com connectgateway.googleapis.com cloudresourcemanager.googleapis.com container.googleapis.com gkeconnect.googleapis.com gkehub.googleapis.com serviceusage.googleapis.com anthos.googleapis.com logging.googleapis.com monitoring.googleapis.com stackdriver.googleapis.com storage-api.googleapis.com storage-component.googleapis.com securetoken.googleapis.com sts.googleapis.com anthos.googleapis.com # to enable APIs" | pv -qL 100
            gcloud --project $GCP_PROJECT services enable gkemulticloud.googleapis.com connectgateway.googleapis.com cloudresourcemanager.googleapis.com container.googleapis.com gkeconnect.googleapis.com gkehub.googleapis.com serviceusage.googleapis.com anthos.googleapis.com logging.googleapis.com monitoring.googleapis.com stackdriver.googleapis.com storage-api.googleapis.com storage-component.googleapis.com securetoken.googleapis.com sts.googleapis.com anthos.googleapis.com 
        elif [ $MODE -eq 3 ]; then
            export STEP="${STEP},4x"
            echo
            echo "*** Nothing to delete ***" | pv -qL 100
...
case ${PLATFORM^^} in
    AWS)
        if [ $MODE -eq 1 ]; then
            export STEP="${STEP},5i"
            echo
            echo "$ \$PROJDIR/aws/eksctl create cluster --name \$AWS_CLUSTER --node-type \$AWS_NODE_TYPE --nodes 3 --nodes-min 3 --nodes-max 5 --region \$AWS_REGION --version 1.26 --spot --full-ecr-access --ssh-access # to create cluster" | pv -qL 100
            echo
            ...
            cd $PROJDIR/aws
            echo "$ $PROJDIR/aws/eksctl create cluster --name $AWS_CLUSTER --node-type $AWS_NODE_TYPE --nodes 3 --nodes-min 3 --nodes-max 5 --region $AWS_REGION --version 1.26 --spot --full-ecr-access --ssh-access # to create cluster" | pv -qL 100
            $PROJDIR/aws/eksctl create cluster --name $AWS_CLUSTER --node-type $AWS_NODE_TYPE --nodes 3 --nodes-min 3 --nodes-max 5 --region $AWS_REGION --version 1.26 --spot --full-ecr-access --ssh-access 
            echo
    ...
    AZURE)
        if [ $MODE -eq 1 ]; then
            export STEP="${STEP},5i"
            echo
            echo "$ $HOME/bin/az aks create --resource-group \$AZ_RESOURCEGROUP --name \$AZURE_CLUSTER --node-count 2 --node-vm-size \$AZURE_NODE_TYPE --enable-cluster-autoscaler --min-count 2 --max-count 4 --enable-addons monitoring --generate-ssh-keys --kubernetes-version 1.26 # to create cluster" | pv -qL 100
            echo
            echo "$ $HOME/bin/az aks nodepool add --resource-group \$AZ_RESOURCEGROUP --cluster-name \$AZURE_CLUSTER --name spotnodepool --priority Spot --eviction-policy Delete --spot-max-price -1 --enable-cluster-autoscaler --min-count 2 --max-count 10 --no-wait # to add spot node pool" | pv -qL 100
            echo      
            echo "$ $HOME/bin/az aks get-credentials -n \$AZURE_CLUSTER -g \$AZ_RESOURCEGROUP # to get container credentials" | pv -qL 100
            echo
            echo "$ kubectl create clusterrolebinding cluster-admin-binding --clusterrole=cluster-admin --user=\"\$(gcloud config get-value core/account)\" # to enable current user to set RBAC rules" | pv -qL 100
            echo
            ...
            export STEP="${STEP},5"
            echo
            echo "$ $HOME/bin/az aks create --resource-group $AZ_RESOURCEGROUP --name $AZURE_CLUSTER --node-count 4 --node-vm-size $AZURE_NODE_TYPE --enable-cluster-autoscaler --min-count 4 --max-count 6 --enable-addons monitoring --generate-ssh-keys --kubernetes-version 1.26 --enable-addons http_application_routing # to create cluster" | pv -qL 100
            $HOME/bin/az aks create --resource-group $AZ_RESOURCEGROUP --name $AZURE_CLUSTER --node-count 4 --node-vm-size $AZURE_NODE_TYPE --enable-cluster-autoscaler --min-count 4 --max-count 6 --enable-addons monitoring --generate-ssh-keys --kubernetes-version 1.26 --enable-addons http_application_routing 
            echo
            echo "$ $HOME/bin/az aks nodepool add --resource-group $AZ_RESOURCEGROUP --cluster-name $AZURE_CLUSTER --name spotnodepool --priority Spot --eviction-policy Delete --spot-max-price -1 --enable-cluster-autoscaler --min-count 2 --max-count 10 --no-wait # to add spot node pool" | pv -qL 100
            $HOME/bin/az aks nodepool add --resource-group $AZ_RESOURCEGROUP --cluster-name $AZURE_CLUSTER --name spotnodepool --priority Spot --eviction-policy Delete --spot-max-price -1 --enable-cluster-autoscaler --min-count 1 --max-count 10 --no-wait
            echo      
            echo "$ $HOME/bin/az aks get-credentials -n $AZURE_CLUSTER -g $AZ_RESOURCEGROUP # to get container credentials" | pv -qL 100
            $HOME/bin/az aks get-credentials -n $AZURE_CLUSTER -g $AZ_RESOURCEGROUP
            echo
            echo "$ kubectl create clusterrolebinding cluster-admin-binding --clusterrole=cluster-admin --user=\"\$(gcloud config get-value core/account)\" # to enable current user to set RBAC rules" | pv -qL 100
            kubectl create clusterrolebinding cluster-admin-binding --clusterrole=cluster-admin --user="$(gcloud config get-value core/account)"
            echo
            ...
            export STEP="${STEP},5x"
            echo
            echo "$ $HOME/bin/az aks delete --name $AZURE_CLUSTER --resource-group $AZ_RESOURCEGROUP # to delete cluster" | pv -qL 100
            $HOME/bin/az aks delete --name $AZURE_CLUSTER --resource-group $AZ_RESOURCEGROUP
        else
            export STEP="${STEP},5i"
            echo
...
if [ $MODE -eq 1 ]; then
    ...
    case ${PLATFORM^^} in
        ...
        AZURE)
            echo
            echo "$ $HOME/bin/az aks get-credentials -n \$AZURE_CLUSTER -g \$AZ_RESOURCEGROUP # to retrieve cluster credentials" | pv -qL 100
            echo
...
EOF
            ...
            $PROJDIR/kubectx eks
            echo
            echo "$ gcloud container attached clusters register $AWS_CLUSTER --location=$GCP_REGION --fleet-project=$GCP_PROJECT --platform-version=1.26.0-gke.2 --distribution=eks --issuer-url=$OIDC_URL --context=$(kubectl config current-context) --admin-users=$(gcloud config get-value core/account) --kubeconfig=~/.kube/config --description=$AWS_CLUSTER --logging=SYSTEM,WORKLOAD # to register cluster" | pv -qL 100
            gcloud container attached clusters register $AWS_CLUSTER --location=$GCP_REGION --fleet-project=$GCP_PROJECT --platform-version=1.26.0-gke.2 --distribution=eks --issuer-url=$OIDC_URL --context=$(kubectl config current-context) --admin-users=$(gcloud config get-value core/account) --kubeconfig=~/.kube/config --description=$AWS_CLUSTER --logging=SYSTEM,WORKLOAD 
            echo
        ...
        AZURE)
            ...
            $PROJDIR/kubectx aks
            echo
            echo "$ gcloud container attached clusters register $AZURE_CLUSTER  --location=$GCP_REGION --fleet-project=$GCP_PROJECT --platform-version=1.26.0-gke.2 --distribution=aks --context=$(kubectl config current-context) --has-private-issuer --admin-users=$(gcloud config get-value core/account) --kubeconfig=~/.kube/config --logging=SYSTEM,WORKLOAD # enable logging" | pv -qL 100
            gcloud container attached clusters register $AZURE_CLUSTER  --location=$GCP_REGION --fleet-project=$GCP_PROJECT --platform-version=1.26.0-gke.2 --distribution=aks --context=$(kubectl config current-context) --has-private-issuer --admin-users=$(gcloud config get-value core/account) --kubeconfig=~/.kube/config --logging=SYSTEM,WORKLOAD 
            echo
            echo "$ gcloud beta container fleet memberships generate-gateway-rbac --membership=$AZURE_CLUSTER --role=clusterrole/cluster-admin --users=$CLOUDBUILD_SA --project=$GCP_PROJECT --context $(kubectl config current-context) --kubeconfig $HOME/.kube/config --apply # to enable clusters to authorize requests from cloud build" | pv -qL 100
    ...
    case ${PLATFORM^^} in
        ...
        AZURE)
            ...
            echo
            echo "$ gcloud container attached clusters delete $AZURE_CLUSTER --location=europe-west1 --context=aks # to delete cluster" | pv -qL 100
            gcloud container attached clusters delete $AZURE_CLUSTER --location=europe-west1 --context=aks
        ;;
        GCP)
            echo
            echo "$ gcloud container clusters get-credentials $GCP_CLUSTER --zone $GCP_ZONE # to retrieve the credentials for cluster" | pv -qL 100
            ...
            gcloud container fleet memberships unregister $GCP_CLUSTER --context=gke
        ;;
    esac
else
    export STEP="${STEP},6i"
